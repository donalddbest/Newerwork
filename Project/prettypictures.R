...
sum(avegenes)
png('newECDF.png')
plot(ecdf(greedy$TotalProfit),verticals=TRUE, do.points=FALSE, col = 'blue',xlab = 'Total Profit',ylab = 'Empirical Cumulative Density', main = 'Comparing ECDFs of the Decision Rules')
lines(ecdf(genes$TotalProfit),verticals=FALSE, do.points=FALSE, col = 'darkred',lwd = 3)
legend(110000,1,c('Maximize Expected Profits','Maximize Expected Genes'),col = c('blue','darkred'),lty = c(1,2), lwd = c(1,3))
dev.off()
