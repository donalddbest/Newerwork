...
df = pandas.read_csv('../Data/babies-first-names-1980-1989.csv')
...
data = list(csv.reader(open('../Data/newprices.csv')))
...
profitfile = open('../Data/probprofitfile.csv', "a")
mffile = open('../Data/mffile.csv', "a")
...
class Snake:
	...
	def __init__(self, name, parent1 = 0, parent2 = 0, traits = 0, sex = 0, age = 0):
		...
		if (sex == 0):
			...
			if (sexnum == 0):
				self.sex = 'Male'
				self.numTimesBreedable = numbreed
				self.ageBreedable = 1
			else:
				self.sex = 'Female'
...
def breed(snake1,snake2):
	...
	if (snake1.numTimesBreedable >= snake1.numTimesBred + 1 and snake2.numTimesBreedable >= snake2.numTimesBred + 1) and (snake1.age>=snake1.ageBreedable and snake2.age>=snake2.ageBreedable) and (snake1.sex != snake2.sex):
		if (yesno == 0):
			...
			numbabies = np.floor(np.random.triangular(lowerbound,maxprob,upperbound, 1))
...
def tick(transitive):
	...
	if(breedingrule < 2):
		...
		for i in modelr.I:
			modelr.row.add(sum(modelr.x[i,j] for j in modelr.J)<=numbreed)
	...
	if breedingrule == 2:
		...
		for i in modelg.I:
			modelg.row.add(sum(modelg.x[i,j] for j in modelg.J)<=numbreed)
		...
		for i in modeln.I:
			modeln.row.add(sum(modeln.x[i,j] for j in modeln.J)<=numbreed)
	...
	if breedingrule == 0:
		...
		def obj_expression(model):
			return 6*sum(sum(model.x[i,j]*utmat[i+1,j+1] for j in model.J) for i in model.I) + sum(-cost*model.y[i] for i in model.I)+sum(-cost*model.z[j] for j in model.J)
		model.OBJ = Objective(rule = obj_expression, sense = maximize)
		def capcons(model):
			return sum(model.y[i] for i in model.I) + sum(model.z[j] for j in model.J) <= capacity
		model.row = ConstraintList()
		for i in model.I:
			model.row.add(sum(model.x[i,j] for j in model.J)<=numbreed)
		...
		for i in range(0,len(sellf)):
			rev = rev + sellf[i].price
		snakes = keptm + keptf
		malefemale.append([len(keptm),len(keptf),numbreed])
		return rev - cost*len(snakes)
...
for j in range(0,200):
	# The commented code below implements the snake breeder's rule
	# nameindex = 0
	# breedingrule = 2
	# initsnakes()
	# profit = []
	...
	initsnakes()
	profit = []
	if( (breedingrule == 0 or breedingrule == 2) and (int(lowerbound) == lowerbound) and (int(upperbound) == upperbound) and (int(maxprob)== maxprob) and (int(capacity) == capacity) and (int(numbreed)==numbreed) and (cost >= 0) and (p>0) and (p<1) and (lowerbound <= maxprob) and (maxprob <= upperbound) and (lowerbound>=0) and (capacity > 0) and (numbreed >=1)):
		for i in range(0,numyearssimulated):
			profit.append(tick(transitive))
			transitive = list(snakes)
		for item in profit:
		...
		print 'Please sanity check your parameter values. Most should be non-negative integers. breedingrule will only accept 0 or 2. p should be strictly between 0 and 1. Cost should be non-negative.'
	# Do Not Change!!!!!
	nameindex = 0
	breedingrule = 0
	lowerbound = 3
	...
	if( (breedingrule == 0 or breedingrule == 2) and (int(lowerbound) == lowerbound) and (int(upperbound) == upperbound) and (int(maxprob)== maxprob) and (int(capacity) == capacity) and (int(numbreed)==numbreed) and (cost >= 0) and (p>0) and (p<1) and (lowerbound <= maxprob) and (maxprob <= upperbound) and (lowerbound>=0) and (capacity > 0) and (numbreed >=1)):
		...
		print 'Please sanity check your parameter values. Most should be positive integers. breedingrule will only accept 0 or 2. p should be strictly between 0 and 1. Cost should be non-negative.'

if numbreed == 6:
	for i in range(0,len(malefemale)):
		mffile.write('%s,' % malefemale[i][0])
		mffile.write('%s,' % malefemale[i][1])
		mffile.write('%s\n' % malefemale[i][2])
